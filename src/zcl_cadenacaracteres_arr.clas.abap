...
CLASS zcl_cadenacaracteres_arr IMPLEMENTATION.
  ...
  data(lv_num) = strlen( ' daniel ' ). "esto cuenta las letras y espacios.
  ...
  lv_num = count_any_of( val = lv_var sub = 'ex' )."count_any_of "devuelve nº caracteres sin discriminar mayusculas o minusculas donde aparece "ex".

        out->write( lv_num ).

  lv_num = count_any_not_of( val = lv_var sub = 'ex' )."cuenta lo que no sea "ex" en esa palabra, discrimina mayusculas minusculas.
